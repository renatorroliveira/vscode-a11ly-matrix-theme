...
Research date: 2026-09-15, re-verified the same day against the registry and
`pnpm audit` (see "Transitive advisories"). Runtime: Node v24.21.0, npm 11.19.0,
corepack 0.36.0.
...
| @vscode/vsce           | 3.9.2   | 2026-06-03 | 4.0.0 (2026-09-14)                                       | 4.0.0 is one day old and raises the Node baseline to 22; eligible on 2026-10-14. It swaps keytar for @napi-rs/keyring and drops @secretlint/node but keeps typed-rest-client, so it would not remove the qs advisory below. | none                                                                 | engines `>= 20`.                                                                                                         |
...
`pnpm audit --prod=false` on the first lockfile reported seven findings, all
reached only through `@vscode/vsce` 3.9.2 and all caused by rule 1: the patched
releases were younger than 30 days, so `minimumReleaseAge` held the tree on
vulnerable versions.

| Package  | Vulnerable | Path                                             | Advisories                                                                         | First patched | Published  | Waiver ends |
| -------- | ---------- | ------------------------------------------------ | ---------------------------------------------------------------------------------- | ------------- | ---------- | ----------- |
| fast-uri | 3.1.5      | vsce > @secretlint/node > ajv                    | GHSA-5jgf-p345-68v8, GHSA-f65p-4m7j-42xc, GHSA-fph4-wmhf-6fwf, GHSA-jqff-g426-hqxp | 3.1.6         | 2026-08-23 | 2026-09-22  |
| js-yaml  | 4.3.1      | vsce > @secretlint/node > rc-config-loader       | GHSA-2883-xcg3-v3hh                                                                | 4.3.2         | 2026-08-26 | 2026-09-25  |
| qs       | 6.15.3     | vsce > azure-devops-node-api > typed-rest-client | GHSA-x5fp-wj9c-mxmx, GHSA-4mjr-xmp4-gh2g                                           | 6.16.0        | 2026-08-29 | 2026-09-28  |

The three packages were added to `minimumReleaseAgeExclude`. A bare exclusion
lets pnpm pick the newest release, which for fast-uri was 3.1.8 (published
2026-09-15, the same day); the scoped `overrides: 'fast-uri@3': 3.1.6` entry
pins the first patched release instead. js-yaml 4.3.2 and qs 6.16.0 are the
newest in their ranges, so no override is needed. Remove each waiver (and the
override) on the date in the last column; the age rule then selects the same
versions on its own.

Direct dependencies were re-checked on the same date: every pin is still the
newest release that satisfies rule 1. typescript-eslint 8.70.0 (2026-09-07) still
declares `typescript >=4.8.4 <6.1.0`, so TypeScript 7.0.2 remains blocked by
rule 5 even though it satisfies rule 1.
