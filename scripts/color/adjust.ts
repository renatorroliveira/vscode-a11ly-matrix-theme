...
export function nudgeToContrast(
  ...
  if (ratioBefore >= targetRatio) {
    return { original: foregroundHex, adjusted: foregroundHex, ratioBefore, ratioAfter: ratioBefore, changed: false, satisfied: true };
  }
  const adjusted = searchLightness(foreground, background, targetRatio);
  if (adjusted === undefined) {
    return { original: foregroundHex, adjusted: foregroundHex, ratioBefore, ratioAfter: ratioBefore, changed: false, satisfied: false };
  }
  const ratioAfter = contrastRatio(adjusted, background);
  return { original: foregroundHex, adjusted: formatHex({ ...adjusted, a: 1 }), ratioBefore, ratioAfter, changed: true, satisfied: true };
}
...
function searchLightness(foreground: Rgb, background: Rgb, targetRatio: number): Rgb | undefined {
  ...
  return (
    searchTowards(hsl, preferred, background, targetRatio) ?? searchTowards(hsl, 1 - preferred, background, targetRatio)
  );
}
...
export function fadeOverlayToContrast(
  ...
  if (ratioBefore >= targetRatio) {
    return { original: overlayHex, adjusted: overlayHex, ratioBefore, ratioAfter: ratioBefore, changed: false, satisfied: true };
  }
  let passing = 0;
  ...
  return { original: overlayHex, adjusted: formatHex({ ...overlay, a: alpha }), ratioBefore, ratioAfter, changed: true, satisfied: ratioAfter >= targetRatio };
}
